...
miss = function(column){
  sum(is.na(column))/length(column) * 100 # soma os na da coluna, divide pelo tamanho do numero de colunas *100
# estamos somando os NA da coluna e dividindo pelo total de elementos da coluna  
}

#comando apply serve para voce mostrar onde ira aplicar a função, o 2 ali e para aplicar na coluna e tivesse 1 era na linha, nome da função
...
for(x in c("Adriana", "Bruna", "Carlos", "Eduardo")){
  print(substr(x, 1, 1)) # essa linha, x é a lista dos nomes, 1(começa), (1 termina)
}
