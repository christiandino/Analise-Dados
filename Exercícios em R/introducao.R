...
# time series
ts(1:10, start = 2000)
ts(1:20, frequency = 12, start = c(2000, 1))

# Importando dados
# getwd() para pegar endereco atual
iris = read.table('iris.csv', sep=',', dec='.', skip=0, header=TRUE)
head(iris)
tail(iris)
...
# a?? ?? s?? usar as funcoes
ggplot(iris, aes(x=sepalwidth, y=sepallength )) + geom_point(aes(col=class), size=1)
